...
def generate_mock_data(num_records, unique_values_dict):
    ...
    # Weights for specific columns
    weights_dict = {
        "sex": {"female": 0.7, "male": 0.3},
        "covid19_admission_hospital": {"no": 0.8, "yes": 0.2},
        "covid19_confirmed_case": {"yes": 0.6, "no": 0.4},
        "has_comorbidities": {"yes": 0.5, "no": 0.5},
        "current_dmt": {"yes": 0.4, "never_treated": 0.3, "no": 0.3},
        "ms_type2": {"relapsing_remitting": 0.6, "progressive_MS": 0.3, "other": 0.1}
    }
    ...
    for column, unique_values in unique_values_dict.items():
        ...
        if column in weights_dict:
            # If the column has specified weights, use them
            weights = [weights_dict[column].get(val, 0) for val in unique_values]
            mock_data[column] = random.choices(unique_values, weights=weights, k=num_records)
